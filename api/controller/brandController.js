...
const deletebrand = async (req, res) => {
  try {
    ...
    if (!_id) {
      return res
        .status(400)
        .json({ error: "Invalid request, provide _id for brand deletion" });
    }
    ...
    if (!deletedBrand) {
      return res
        .status(404)
        .json({ error: "Brand not found", data: deletedBrand });
    }

    res
      .status(200)
      .json({ success: true, msg: "Deleted brand and related user products" });
  } catch (error) {
